...
import { register, evaluateAll } from '@prosdevlab/experience-sdk';
      ...
      <main className="max-w-6xl mx-auto px-6 py-16">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-zinc-600 dark:text-zinc-400 mb-8">
            This page demonstrates how the SDK evaluates and shows banners, with full explainability of the decision process.
          </p>
          <p className="text-sm text-zinc-500 dark:text-zinc-500 mb-4">
            Banners target URLs containing &quot;/shop&quot; and respect frequency capping.
          </p>
                    ...
                    <div className="flex-1">
                      ...
                      {decision.experienceId && (
                        <div className="text-xs text-zinc-500 dark:text-zinc-500 mt-1">
                          Experience ID: <code className="px-1 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded">{decision.experienceId}</code>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="p-6 bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800">
                ...
                <div className="space-y-3">
                  {decision.trace.map((step, idx) => (
                    <div key={idx} className="p-3 bg-zinc-50 dark:bg-zinc-800 rounded border border-zinc-200 dark:border-zinc-700">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${step.passed ? 'bg-green-500' : 'bg-red-500'}`} />
                      ...
                      <div className="text-xs space-y-2 ml-4">
                        <div>
                          <span className="text-zinc-500 dark:text-zinc-500 font-medium">Input:</span>
                          <JsonDisplay data={step.input} />
                        </div>
                        <div>
                          <span className="text-zinc-500 dark:text-zinc-500 font-medium">Output:</span>
                ...
                <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-xs text-zinc-500 dark:text-zinc-500">Total Duration</div>
                      <div className="text-sm font-medium text-zinc-900 dark:text-zinc-50">
                        {decision.metadata.totalDuration}ms
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-zinc-500 dark:text-zinc-500">Experiences Evaluated</div>
                      <div className="text-sm font-medium text-zinc-900 dark:text-zinc-50">
                        {decision.metadata.experiencesEvaluated}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-zinc-500 dark:text-zinc-500">Evaluated At</div>
                      <div className="text-sm font-medium text-zinc-900 dark:text-zinc-50">
